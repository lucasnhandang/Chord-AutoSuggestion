...
function setupEventListeners() {
    ...
    // File input change
    document.getElementById("jsonFileInput").addEventListener(
        "change",
        handleFileImport,
    );

    // Play/Pause button - dual functionality
    document.getElementById("playPauseBtn").addEventListener(
        "click",
        handlePlayPauseClick,
    );

    // Reset offset button
    document.getElementById("resetOffsetBtn").addEventListener(
        "click",
        handleResetOffset,
    );
    ...
    // Chord edit controls
    document.getElementById("deleteChordBtn").addEventListener(
        "click",
        handleDeleteChord,
    );
    document.getElementById("addChordBtn").addEventListener(
        "click",
        handleAddChord,
    );
...
function selectSong(songKey, songElement) {
    ...
    currentSong = { _name: songKey, ...songsData[songKey] };
...
function extractVideoId(url) {
    ...
    try {
        ...
        if (/(youtube\.com)$/.test(u.hostname)) {
            ...
            if (
                eidx !== -1 && parts[eidx + 1] &&
                /^[a-zA-Z0-9_-]{11}$/.test(parts[eidx + 1])
            ) return parts[eidx + 1];
        }
    } catch {}
...
function onPlayerStateChange(event) {
    ...
    if (
        event.data === YT.PlayerState.PLAYING && !isPlaying &&
        offsetTime !== null
    ) {
...
function setOffset() {
    ...
    offsetTime = currentSong.offset ?? 0;
    offsetSet = true;

    // Seek to beginning of video
    player.seekTo(offsetTime, true);
    ...
    console.log(
        "Offset set to:",
        offsetTime,
        "and video started from beginning",
    );
}
...
function startChordProgression() {
    ...
    chordInterval = setInterval(() => {
        ...
        const chordIndex = Math.floor(elapsedTime / 60.0 * currentSong.bpm);
    ...
    }, 5); // Update every 5ms for smooth animation
...
function updateActiveChord(index) {
    ...
    if (prevChordIndex >= 0 && currentSong && currentSong.chords) {
        ...
        if (
            prevChordValue && currentChordValue &&
            prevChordValue !== currentChordValue
        ) {
            isChordChange = true;
        }
    ...
    if (currentChordIndex >= 0) {
        const prevChord = document.querySelector(
            `[data-chord-index="${currentChordIndex}"]`,
        );
        if (prevChord) {
            prevChord.classList.remove(
                "active",
                "chord-transition",
                "different-chord",
            );
    ...
    const currentChord = document.querySelector(
...
function handleResetOffset() {
    ...
    songsData[currentSong._name].offset = currentTime;
    ...
    showToast(
        `Offset reset to ${formatTime(offsetTime)} and video paused`,
        "success",
    );
}
...
function renderChordTrack() {
    ...
    console.log(
        "First 10 chords:",
        JSON.stringify(currentSong.chords.slice(0, 10)),
    );
    ...
    if (selectedChordIndex >= currentSong.chords.length) {
        console.log(
            "Resetting selectedChordIndex from",
            selectedChordIndex,
            "to -1 (out of bounds)",
        );
        selectedChordIndex = -1;
    ...
    currentSong.chords.forEach((chord, index) => {
        ...
        chordText.textContent =
            (index == 0 || chord !== currentSong.chords[index - 1]) ? chord : "";
    ...
    if (
        ...
        selectedChordIndex < currentSong.chords.length
    ) {
        const selectedChord = document.querySelector(
            `[data-chord-index="${selectedChordIndex}"]`,
        );
        if (selectedChord) {
            selectedChord.classList.add("selected");
            const chordEditControls = document.getElementById(
                "chordEditControls",
            );
            if (chordEditControls) {
...
function startEditingChord(chordBox, index) {
    ...
    input.type = "text";
    input.value = currentSong.chords[index];
    ...
    const saveEdit = () => {
        ...
        if (index == 0 || newValue !== currentSong.chords[index - 1]) {
            newChordText.textContent = newValue;
        } else {
            newChordText.textContent = "";
        }
        newChordText.className = "chord-text";
        chordBox.appendChild(newChordText);

        const nextChordBox = document.querySelector(
            `[data-chord-index="${index + 1}"]`,
        )
        if (nextChordBox) {
            const nextSpan = nextChordBox.querySelector('*')
            if (newValue === currentSong.chords[index + 1])
                nextSpan.textContent = "";
            else 
                nextSpan.textContent = currentSong.chords[index + 1];
        }
    };
...
function updateChord(index, newValue) {
    ...
    console.log(currentSong._name);
    songsData[currentSong._name].chords[index] = newValue;
...
function handleDeleteChord() {
    ...
    console.log(
        "Chords before delete:",
        JSON.stringify(
            currentSong.chords.slice(
                Math.max(0, deletedIndex - 2),
                deletedIndex + 3,
            ),
        ),
    );
...
function handleAddChord() {
    ...
    console.log(
        "songsData[currentSong._name] exists:",
        !!songsData[currentSong._name],
    );
    ...
    if (
        ...
        songsData[currentSong._name].chords.splice(insertIndex, 0, newChord);
    } else {
    ...
    console.log(
        "Current chords after add:",
        JSON.stringify(
            currentSong.chords.slice(insertIndex - 2, insertIndex + 3),
        ),
    ); // Show surrounding chords

    // Ensure currentSong is in sync with songsData
    if (currentSong._name && songsData[currentSong._name]) {
        currentSong.chords = songsData[currentSong._name].chords;
    ...
    setTimeout(() => {
        ...
        if (allChordBoxes.length > insertIndex) {
            ...
            setTimeout(() => {
                const newChordBox = document.querySelector(
                    `[data-chord-index="${insertIndex}"]`,
                );
